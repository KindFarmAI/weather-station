import { GeoLocation, DailyForecast, HourlyForecast, ForecastSource, ArchiveDay } from './types';
...
export async function searchLocations(q: string): Promise<GeoLocation[]> {
  ...
  const p = new URLSearchParams({q,format:'json',limit:'8','accept-language':'ru,en',addressdetails:'1'});
...
function pD(d:any):DailyForecast[]{
  ...
  return d.daily.time.map((t:string,i:number)=>({
    ...
    windDir:null, humidityMax:d.daily.relative_humidity_2m_max?.[i]??null, humidityMin:d.daily.relative_humidity_2m_min?.[i]??null,
...
function mkE(id:string,nm:string,md:string,msg:string):ForecastSource{return{id,name:nm,model:md,daily:[],hourly:[],loaded:false,error:msg,loading:false};}

export async function fetchECMWF(lat:number,lon:number,days:number=16):Promise<ForecastSource>{
  try{const p=new URLSearchParams({latitude:String(lat),longitude:String(lon),forecast_days:String(days),models:'ecmwf_ifs',daily:DF,hourly:HF,timezone:'auto'});
  const r=await fetch(FC+'?'+p);if(!r.ok)throw new Error('HTTP '+r.status);const d=await r.json();
  return{id:'ecmwf',name:'ECMWF IFS',model:'ecmwf_ifs',daily:pD(d),hourly:pH(d),loaded:true,error:null,loading:false};}
  catch(e:any){return mkE('ecmwf','ECMWF IFS','ecmwf_ifs',e.message);}
}
export async function fetchGFS(lat:number,lon:number,days:number=16):Promise<ForecastSource>{
  try{const p=new URLSearchParams({latitude:String(lat),longitude:String(lon),forecast_days:String(days),models:'gfs_seamless',daily:DF,hourly:HF,timezone:'auto'});
  const r=await fetch(FC+'?'+p);if(!r.ok)throw new Error('HTTP '+r.status);const d=await r.json();
  return{id:'gfs',name:'GFS (Windy)',model:'gfs_seamless',daily:pD(d),hourly:pH(d),loaded:true,error:null,loading:false};}
  catch(e:any){return mkE('gfs','GFS (Windy)','gfs_seamless',e.message);}
}
export async function fetchICON(lat:number,lon:number,days:number=7):Promise<ForecastSource>{
  try{const p=new URLSearchParams({latitude:String(lat),longitude:String(lon),forecast_days:String(days),models:'icon_eu',daily:DF,timezone:'auto'});
  const r=await fetch(FC+'?'+p);if(!r.ok)throw new Error('HTTP '+r.status);const d=await r.json();
  return{id:'icon',name:'ICON-EU',model:'icon_eu',daily:pD(d),hourly:[],loaded:true,error:null,loading:false};}
  catch(e:any){return mkE('icon','ICON-EU','icon_eu',e.message);}
}

// ECMWF IFS analysis via forecast API — much better than ERA5 for recent data
export async function fetchRecentArchive(lat:number,lon:number,days:number=14):Promise<ArchiveDay[]>{
  try{const p=new URLSearchParams({latitude:String(lat),longitude:String(lon),past_days:String(days),forecast_days:'1',
    models:'ecmwf_ifs',daily:'temperature_2m_max,temperature_2m_min,precipitation_sum,wind_speed_10m_max',timezone:'auto'});
  const r=await fetch(FC+'?'+p);if(!r.ok)return[];const d=await r.json();if(!d.daily?.time)return[];
...
// ERA5 fallback for older dates
export async function fetchERA5Archive(lat:number,lon:number,s:string,e:string):Promise<ArchiveDay[]>{
  try{const p=new URLSearchParams({latitude:String(lat),longitude:String(lon),start_date:s,end_date:e,
    daily:'temperature_2m_max,temperature_2m_min,precipitation_sum,wind_speed_10m_max',timezone:'auto'});
  const r=await fetch(AR+'?'+p);if(!r.ok)return[];const d=await r.json();if(!d.daily?.time)return[];
...
export async function fetchYandex(slug:string):Promise<ForecastSource>{
  ...
  try{const url='https://yandex.ru/pogoda/'+encodeURIComponent(slug)+'/details?via=mstack';
    ...
    const days=parseYa(html);return{...base,daily:days,loaded:days.length>0,error:days.length===0?'Парсинг':'',loading:false};}
...
function parseYa(html:string):DailyForecast[]{
  const out:DailyForecast[]=[];const mm:any={'января':'01','февраля':'02','марта':'03','апреля':'04','мая':'05','июня':'06','июля':'07','августа':'08','сентября':'09','октября':'10','ноября':'11','декабря':'12'};
  const yr=new Date().getFullYear();const blocks=html.split('weather-table__row');
  for(let i=1;i<blocks.length;i++){const b=blocks[i];const dm=b.match(/(\d{1,2})\s+(января|февраля|марта|апреля|мая|июня|июля|августа|сентября|октября|ноября|декабря)/i);
    if(!dm)continue;const ts=Array.from(b.matchAll(/temp__value[^"]*"[^>]*>([+-]?\d+)/g));
    const pm=b.match(/precipitation[^"]*"[^>]*>\s*(?:<[^>]*>)*\s*([\d,.]+)\s*мм/);
    const mo=mm[dm[2].toLowerCase()]||'01';const da=dm[1].padStart(2,'0');
    out.push({date:yr+'-'+mo+'-'+da,tempMax:ts.length?+ts[0][1]:null,tempMin:ts.length>1?+ts[ts.length-1][1]:ts.length?+ts[0][1]:null,
      precipSum:pm?+pm[1].replace(',','.'):0,windMax:null,windDir:null,humidityMax:null,humidityMin:null,pressureMax:null,pressureMin:null,
      uvIndexMax:null,precipProb:null,sunrise:null,sunset:null,weatherCode:null,windGusts:null,et0:null});}
  return out;
}

export function formatDate(ds:string):string{const d=new Date(ds+'T00:00:00');const w=['Вс','Пн','Вт','Ср','Чт','Пт','Сб'];const m=['янв','фев','мар','апр','мая','июн','июл','авг','сен','окт','ноя','дек'];return d.getDate()+' '+m[d.getMonth()]+', '+w[d.getDay()];}
export function formatDateFull(ds:string):string{const d=new Date(ds+'T00:00:00');const m=['января','февраля','марта','апреля','мая','июня','июля','августа','сентября','октября','ноября','декабря'];return d.getDate()+' '+m[d.getMonth()]+' '+d.getFullYear();}
export function getPrecipClass(mm:number|null):string{if(mm==null||mm===0)return'pl';if(mm<5)return'pl';if(mm<20)return'pm';return'ph';}
export function todayStr():string{const d=new Date();return d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0');}
export function daysAgo(n:number):string{const d=new Date();d.setDate(d.getDate()-n);return d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0');}
export function fmt(v:number|null|undefined,dec:number=1):string{return v!=null?v.toFixed(dec):'—';}
export function pbw(v:number|null):number{return(v==null||v===0)?0:Math.min(100,Math.max(2,v*2));}
